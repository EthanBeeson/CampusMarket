...
python -m scripts.seed_global_db
...
- Uploads default to `shared_uploads/` (tracked). Keep listing images in `shared_uploads/listing_images/` and profile pics in `shared_uploads/profile_pictures/` so teammates get them on pull. If you prefer not to commit images, set `UPLOADS_BASE_DIR` to a shared drive/bucket instead.
